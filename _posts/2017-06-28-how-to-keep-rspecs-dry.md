...
One of the keys is to keep them [DRY](http://wiki.c2.com/?DontRepeatYourself).
...
A class with one depencency, that responds to one method which takes one argument.
...
What I observe, developers tend to write specs in a structure more or less like:
...
Different contexts are covered. Subjects are described.
I believe that by looking on it you can tell what is going on there.

However there are some __repetitions__ which violates DRY principle:

- Tested class name `Foo` is used few times (in lines: 1, 9, 15, 23, 29).
- Knowledge about how to instantialize tested class is duplicated in each example (lines: 9, 15, 23, 29).
- Knowledge about how to call tested method is duplicated as well (lines: 10, 16, 24, 30).

In other words, if one of the followings change:
- class name, 
- way of initializing (more/less dependency),
- method name, 
- arguments number.

..if one of the above change, 4-5 lines would have to be changed to keep the tests up to date.

It's not _that_ problematic, but still, improving makes sense.
...
In different _contexts_ different dependencies are used to instantialize tested class. They can be declared after declaring the _subject_.
We can describe how to build _subject_ in line 3 before declaring it's dependency (line 6), because of _subjest_'s lazy evaluation
(it's evaluated in line 7, not 3).
...
I mean instead of duplicating `subject.call(argument)` in each expectation, we can move calling this method into the
_subject_, because it's the same in each case (line 5 in below _final_ listing).


### Final version

- Tested class name is used once (line: 1 - [described_class](https://relishapp.com/rspec/rspec-core/docs/metadata/described-class)).
- Knowledge how to instantialize tested class is in one place (line: 2).
- Knowledge how to call tested method is in one place, following its _describe_ block (line: 5).
- _let_ blocks are declared within their _contexts_ (lines: `8 and 22`, `11 and 16`, `25 and 30`).
- Read it from top to bottom and you will see a structure __descending from general to detailed__.
